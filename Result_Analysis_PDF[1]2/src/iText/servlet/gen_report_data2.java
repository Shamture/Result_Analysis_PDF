...
*/
   			...
   			sql="SELECT *,(total/`db_pdf2text`.`result_data`.max*100) FROM result_data ,student where result_data.PRN = student.prnno and result_data.total <> '--' AND result_data.max IS NOT NULL and (result_data.total/result_data.max*100)"+resultop+" "+resultid+" group by prn order by total  desc";
   		if(!resultid.equals("any") && !branchid.equals("any"))
   			sql="SELECT *,(total/`db_pdf2text`.`result_data`.max*100) FROM result_data , student where branch = '"+branchid+"' and result_data.PRN = student.prnno and result_data.total <> '--' AND result_data.max IS NOT NULL and (result_data.total/result_data.max*100)"+resultop+" "+resultid+" group by prn order by total  desc";
   		else if(!branchid.equals("any"))
   			sql="SELECT *,(total/`db_pdf2text`.`result_data`.max*100) FROM result_data , student where branch = '"+branchid+"' and result_data.PRN = student.prnno and result_data.total <> '--' AND result_data.max IS NOT NULL and (result_data.total/result_data.max*100)"+resultop+" "+resultid+" group by prn order by total  desc";
   		else if(!resultid.equals("any"))
			sql="SELECT *,(total/`db_pdf2text`.`result_data`.max*100) FROM result_data , student where result_data.PRN = student.prnno and result_data.total <> '--' AND result_data.max IS NOT NULL and (result_data.total/result_data.max*100)"+resultop+" "+resultid+" group by prn order by total desc ";
